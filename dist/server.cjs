...
async function startServer() {
  ...
  app.post("/api/ai/generate", async (req, res) => {
    ...
    try {
      ...
      const isQuota = error?.status === "RESOURCE_EXHAUSTED" || error?.status === 429 || error?.message?.includes("RESOURCE_EXHAUSTED") || error?.message?.includes("429") || error?.message?.includes("Quota exceeded");
      if (!isQuota) {
      ...
      res.status(isQuota ? 429 : 500).json({
        error: isQuota ? "Gemini AI quota exceeded. Please try again in a few minutes." : error?.message || "Failed to generate content from Gemini",
  ...
  app.post("/api/ai/generate-image", async (req, res) => {
    ...
    try {
      ...
      const stylePrompt = style === "meme" ? "Meme format, high contrast, text-ready layout, humorous gaming aesthetic." : `Style: ${style}. High quality, detailed visual, ${style} aesthetic, cinematic lighting.`;
      const fullPrompt = `${prompt}. ${stylePrompt}`;
      const targetModel = model && (model.includes("3.1") || model.includes("pro-image") || model.includes("lite-image")) ? model : "gemini-3.1-flash-lite-image";
      let imageUrl = null;
      let contentResponse;
      try {
      ...
      return res.json({
        ...
        aspectRatio: targetAspectRatio,
        captions: [
          `\u{1F525} Newly created visual: "${prompt}"! #${style} #nxclip #creators`
        ],
        hashtagSets: [
      ...
      const isQuota = error?.status === "RESOURCE_EXHAUSTED" || error?.status === 429 || error?.message?.includes("RESOURCE_EXHAUSTED") || error?.message?.includes("429") || error?.message?.includes("Quota exceeded") || error?.message?.includes("quota") || error?.response?.status === 429 || error?.response?.data?.error?.status === "RESOURCE_EXHAUSTED";
      if (!isQuota) {
        ...
        try {
          ...
          if (pexelsRes.ok) {
            ...
            if (pexelsData.photos && pexelsData.photos.length > 0) {
              ...
              const contentId2 = `pexels_${Date.now()}_${Math.random().toString(36).substring(2, 7)}`;
              ...
              return res.json({
                ...
                aspectRatio: targetAspectRatio,
                captions: [
                  `\u{1F525} High-performance creator visual: "${prompt}"! #nxclip #gaming #creators`
                ],
                hashtagSets: [
  ...
  app.get("/api/docs/migration-guide", async (req, res) => {
    try {
      ...
      const filePath = import_path.default.join(process.cwd(), "docs", "nxclip-migration-guide.md");
  ...
  app.all("/api/gateway-proxy/*", async (req, res) => {
    ...
    const targetGatewayUrl = req.headers["x-target-gateway-url"] || "https://api-gateway-216098834386.us-central1.run.app";
    ...
    const sendErrorResponse = (statusCode, message, code) => {
    ...
    if (req.method === "POST" && req.body && typeof req.body === "object") {
      ...
      if (cleanSubPath === "content/generate") {
        if (!body.prompt || body.prompt.length < 3 || body.prompt.length > 2e3) {
    ...
    if (targetGatewayUrl.includes("localhost") || targetGatewayUrl.includes("127.0.0.1")) {
      const isFeedEngagement = /^(content|users)\/[^\/]+\/(like|comment|comments|follow|profile)/.test(cleanSubPath) || cleanSubPath.startsWith("feed");
      if (isFeedEngagement) {
    ...
    try {
      ...
      Object.entries(req.headers).forEach(([key, value]) => {
        if (!ignoredRequestHeaders.includes(key.toLowerCase()) && value !== void 0) {
      ...
      try {
        response = await (0, import_axios.default)({
          ...
          data: hasBody ? req.body : void 0,
          responseType: "arraybuffer",
          validateStatus: () => true
          // Forward all status codes to client rather than throwing
        });
      } catch (axiosError) {
        const isStaging = finalDestinationUrl.includes("staging-api.nxclip.ai");
        const isNetworkError = axiosError.code === "ENOTFOUND" || axiosError.code === "ECONNREFUSED" || axiosError.code === "ETIMEDOUT" || !axiosError.response || axiosError.message?.includes("ENOTFOUND");
      ...
      Object.entries(response.headers).forEach(([key, value]) => {
        if (!ignoredResponseHeaders.includes(key.toLowerCase()) && value !== void 0) {
