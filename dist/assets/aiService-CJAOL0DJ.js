class r extends Error{constructor(e,n="UNKNOWN"){super(e),this.name="AIError",this.code=n}}async function m(t){return s(t)}async function s(t){try{const e=await fetch("/api/ai/generate",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify(t)});if(!e.ok){const n=await e.json();throw new Error(n.error||"AI Proxy request failed")}return await e.json()}catch(e){return p(e)}}function p(t){console.error("AI Service Error:",t);const e=(t==null?void 0:t.message)||String(t);throw e.includes("429")||e.toLowerCase().includes("rate limit")||e.toLowerCase().includes("quota")?new r("AI is currently under heavy load or rate limited. Please wait a moment and try again.","RATE_LIMIT"):e.includes("SAFETY")||e.toLowerCase().includes("safety filters")||e.toLowerCase().includes("blocked")||e.toLowerCase().includes("candidate")?new r("Content was flagged by AI safety filters. Please try a different, more appropriate prompt.","SAFETY_BLOCK"):e.toLowerCase().includes("api key")||e.includes("403")?new r("AI service configuration error. Please check your API key settings.","API_KEY_ERROR"):e.toLowerCase().includes("invalid")||e.includes("400")?new r("Invalid request sent to the AI. Please refine your prompt and try again.","INVALID_PROMPT"):new r(e||"An unexpected error occurred while connecting to the AI service.","UNKNOWN")}async function f(t,e=[],n=[],o){var i;const l=n.length>0?`

USER CREATIONS CONTEXT:
${n.map(a=>{var c;return`- Type: ${a.type}, Prompt: ${a.prompt||"N/A"}, Status: ${a.status}, Created: ${new Date(((c=a.createdAt)==null?void 0:c.seconds)*1e3).toLocaleDateString()}`}).join(`
`)}`:`

(No creation history found)`,u=`You are the nxclip.ai AI Creator Coach, a premium expert in viral gaming content and channel growth. 
...
(No creator profile found)`}${l}`,d=[...e.map(a=>({role:a.role==="user"?"user":"model",parts:[{text:a.text}]})),{role:"user",parts:[{text:t}]}];return(await s({model:"gemini-3.6-flash",contents:d,config:{systemInstruction:u}})).text}async function y(t){const e=await s({model:"gemini-3.6-flash",contents:`Generate 3 catchy, professional gaming social media captions for an image described as: "${t}". 
    Each caption should include 3-5 relevant gaming hashtags. 
    Return the result as a JSON array of strings.`,config:{responseMimeType:"application/json",responseSchema:{type:"ARRAY",items:{type:"STRING",description:"A single social media caption with hashtags."}}}});return e.text?JSON.parse(e.text):[]}async function A(t){const e=await s({model:"gemini-3.6-flash",contents:`Generate a catchy, short title (max 5 words) for a gaming creation described as: "${t}". 
    Return the result as a JSON object with a single "title" field.`,config:{responseMimeType:"application/json",responseSchema:{type:"OBJECT",properties:{title:{type:"STRING"}},required:["title"]}}});return e.text?JSON.parse(e.text).title:""}export{r as A,A as a,y as b,f as c,m as g};
