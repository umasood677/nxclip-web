...
#### D.3 API Utilization Blueprint (Journey A & B)
*   **Journey A: AI Image Studio**
    *   **Route `POST /content/generate` (JWT):**
        *   *Description:* Previously, generate was an asynchronous background job returning `{ "jobId": "uuid", "status": "queued" }`. It now runs inline synchronously; the API blocks and returns the finished asset data directly in the HTTP response.
        ...
        *   *Success Response:* 
            ```json
            {
              "contentId": "tx_mock_xyz123",
              "cdnUrl": "https://images.unsplash.com/photo-...",
              "captions": [
                "Epic creation featuring: \"A retro cyberpunk gamer overlay\"! 🔥 #creative #visual #cinematic",
                "This new design from Image Studio turned out insane! 🤯👀 #cinematic #art",
                "Rate this generated cinematic piece from 1-10! 👇 #cinematic #concept"
              ],
              "hashtagSets": [
                ["creative", "visual", "cinematic"],
                ["cinematic", "art", "nxclip"],
                ["cinematic", "concept", "design", "foryou"]
              ],
              "id": "tx_mock_xyz123",
              "status": "draft",
            ...
            ```
    *   **Route `POST /content/{id}/publish` (JWT):**
        *   *Description:* Publish the generated draft asset with user-selected hashtags and chosen/edited caption.
        *   *Payload:* `{ "title": "My Artwork", "caption": "Selected caption", "hashtags": ["tag1", "tag2"] }`
        *   *Response:* `{ "success": true, "status": "published" }`

*   **Journey B: Direct Media Upload**
    *   **Route `POST /content/upload-url` (JWT):**
        *   *Description:* Initiates a file upload flow by requesting a pre-signed GCS URL.
        *   *Payload:* `{ "fileName": "epic_gameplay.mp4", "mimeType": "video/mp4", "fileSize": 24117248 }`
        *   *Response:*
            ```json
            {
              "uploadUrl": "https://gcs-mock-upload-bucket.googleapis.com/uploads/epic_gameplay.mp4?GoogleAccessId=...",
              "assetId": "asset_abc123"
            }
            ```
    *   **PUT File Bytes:**
        *   *Flow:* Client performs a binary `PUT` stream directly to the GCS signed URL with the file bytes.
    *   **Route `POST /content/{id}/publish` (JWT):**
        *   *Description:* Publishes the uploaded asset into the feed database.
        *   *Payload:* `{ "title": "My Clip Title", "description": "Clip description" }`
        *   *Response:* `{ "success": true, "status": "published" }`
