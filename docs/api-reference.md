...
There are **two entry paths** into the same publish / moderation pipeline, plus Image Studio **variations** for references and in-place update:
...
| **A2 — Create with references** | `POST /content/generate` + refs | `draft` (sync) | Prompt + owned refs (mine and/or fresh uploads) |
| **A3 — Update existing image** | `POST /content/{id}/regenerate` | same `contentId`, back to `draft` | Current image as base ± prompt ± extra refs |
| **B — Upload clip / asset** | `POST /content/upload-url` + `PUT` file | `draft` | User-uploaded bytes |

Both create journeys end the same way: `POST /content/{id}/publish` → `publishing` → WebSocket moderation → `published`.

**Reference picking (locked MVP):**

| Source | How the SPA gets ids | Request field |
|--------|----------------------|---------------|
| My library | `GET /content/mine` → multi-select owned **image** rows with media | `referenceContentIds` |
| Fresh upload | `POST /content/upload-url` → `PUT` file → use returned `contentId` / `assetId` | `referenceUploadIds` (same ownership rules as content ids) |

Server loads bytes from GCS, verifies ownership + image mime, and sends multimodal parts to AI. FREE plan: max **2** extra refs; PRO+: max **4**. Regenerate always attaches the **target’s current image** as role `base` (does not count against the extra-ref limit).

**Out of MVP:** regenerate of `published` (would need fork); picking other users’ feed posts.
...
|-----|------------------------------------------------------|------------------------|
| `POST /content/generate` | **Yes** | Create the image + captions + hashtagSets (± optional refs) |
| `POST /content/{id}/regenerate` | **No** (edit path) | Update an owned `draft` / `moderation_rejected` image in-place |
...
| `POST /content/{id}/retry-generation` | Only on failure | `generation_failed` (no body; reuses stored prompt) |
| `PATCH /content/{id}` | Only on moderation reject | Edit title/description then re-publish |
| `GET /content/mine` | Optional for create; **required for pick-from-mine refs** | Library / reference picker / status polling |
...
| `POST /content/upload-url` | **No** for prompt-only; **yes** for fresh reference uploads | Journey B and/or reference upload before generate/regenerate |

**Minimum Image Studio journey:** `generate` → (render via `/content/{id}/media`) → `publish` → await `content:moderation_complete`.  
**Create with references:** upload-url (+ PUT) and/or pick from mine → `generate` with `referenceContentIds` / `referenceUploadIds` → publish.  
**Update existing:** open draft from mine → `regenerate` (prompt and/or extra refs) → publish.
...
|------|--------------|---------|----------------------------|
| 1 | `POST /content/generate` | `{ prompt, style?, aspectRatio?, referenceContentIds?, referenceUploadIds? }` | `{ contentId, cdnUrl, thumbnailUrl, captions[3], hashtagSets[3], watermarked }` — status **`draft`**. `cdnUrl` is the **authenticated media route** (not a permanent public GCS URL) |
| 1b (optional) | Before generate: upload refs | `upload-url` + `PUT`, and/or select ids from `GET /content/mine` | Use returned / selected content ids as reference fields |
| 1c (optional) | `POST /content/{id}/regenerate` | `{ prompt?, style?, aspectRatio?, referenceContentIds?, referenceUploadIds? }` | Same Image Studio payload; **same** `contentId`; in-place media overwrite |
...
| 6b | On reject | `PATCH /content/{id}` then re-publish (or `regenerate` then publish) | Status → **`moderation_rejected`** → **`draft`** after PATCH |
...
[*] --generate(+optional refs)--> processing --inline success--> draft --publish--> publishing --feed OK--> published
                      ...
                      +--fail--> generation_failed    +--moderation reject--> moderation_rejected --PATCH--> draft
                                 |                      ^                     |
                                 +--retry-generation----+                     +--regenerate(+optional refs)--> processing --> draft
...
| `generation_failed` | `POST /content/{id}/retry-generation` | Same Image Studio response shape as generate; **no** new prompt/refs |
| Edit successful draft | `POST /content/{id}/regenerate` | New prompt and/or refs; current image is multimodal base |
| Plan limit | — | `403` with `required_plan: "pro"`, `upgrade_url`, `current`, `limit` (daily generate/regenerate limit **10** on FREE; refs FREE max **2**, PRO+ **4**) |
...
| A2 | `GET {{GATEWAY}}/content/{{CONTENT_ID}}/media` (Bearer or cookie) | `302` signed GCS URL (or `200` local stream) — confirm image loads |
| A3 | Connect `/dev/coach-ws` with same JWT (optional) | May see `content:generation_complete`; UI can rely on HTTP response only |
| A4 | Save `contentId` from A1 as `CONTENT_ID` | — |
| A5 | `POST {{GATEWAY}}/content/{{CONTENT_ID}}/publish` body `{ "title": "Mountain Sunset", "caption": "<one of captions>", "hashtags": ["#NxClip"] }` | `200`/`201` — `status: "publishing"` |
| A6 | Watch Socket.IO test page | `content:moderation_complete` with `{ contentId, status: "approved" \| "rejected" }` |
| A7 | `GET {{GATEWAY}}/content/mine/{{CONTENT_ID}}` | Library row; after approve + feed → eventually `published` |
| A7b | `GET {{GATEWAY}}/content/{{CONTENT_ID}}` | `200` only when **published**; `404` while still publishing |

> **Variants:** create-with-refs → [A2 below](#a2-image-studio--create-with-references-postman). Update existing → [A3 below](#a3-image-studio--regenerate--update-existing-postman).
...
#### A2. Image Studio — create with references (Postman)

Use when the user attaches **mine** and/or **fresh upload** images to a new generate.

| Step | Postman | Expect |
|------|---------|--------|
| R0 | Same JWT + `/dev/coach-ws` as section A | Connected |
| R1 (fresh upload ref) | `POST {{GATEWAY}}/content/upload-url` body `{ "fileName": "ref.png", "mimeType": "image/png", "fileSize": <bytes> }` | `201` — `uploadUrl`, `contentId` (= `REF_UPLOAD_ID`) |
| R2 | `PUT {{UPLOAD_URL}}` — Body = **binary** PNG/JPEG; `Content-Type: image/png` | Storage success |
| R3 (mine ref, optional) | `GET {{GATEWAY}}/content/mine` | Pick an owned **image** row with media → `REF_MINE_ID` |
| R4 | `POST {{GATEWAY}}/content/generate` with refs (body below) | `201` — new `contentId` (not the ref ids); captions/hashtags as usual |
| R5 | `GET {{GATEWAY}}/content/{{CONTENT_ID}}/media` | Image loads |
| R6–R8 | Same as A5–A7: publish → WS moderation → `GET /content/mine/{{CONTENT_ID}}` | Ends `published` |

**Postman — generate with references**
...
{
  "prompt": "Same subject standing in a neon cyberpunk city at night",
  "style": "cinematic",
  "aspectRatio": "16:9",
  "referenceUploadIds": ["{{REF_UPLOAD_ID}}"],
  "referenceContentIds": ["{{REF_MINE_ID}}"]
}
```

Either array may be omitted. Combined unique refs: FREE ≤ **2**, PRO+ ≤ **4**. Both id lists must be **owned** image content with `storageKey` (PUT completed for uploads).

**cURL — create with fresh upload reference**

```bash
GATEWAY='https://api-gateway-216098834386.us-central1.run.app'
TOKEN='<accessToken>'
REF_FILE='./ref.png'
REF_SIZE=$(wc -c < "$REF_FILE" | tr -d ' ')

UP=$(curl -s -X POST "$GATEWAY/content/upload-url" \
  -H "Authorization: Bearer $TOKEN" -H 'Content-Type: application/json' \
  -d "{\"fileName\":\"ref.png\",\"mimeType\":\"image/png\",\"fileSize\":$REF_SIZE}")
REF_UPLOAD_ID=$(echo "$UP" | jq -r '.contentId')
UPLOAD_URL=$(echo "$UP" | jq -r '.uploadUrl')

curl -s -X PUT "$UPLOAD_URL" -H 'Content-Type: image/png' --data-binary @"$REF_FILE"

GEN=$(curl -s -X POST "$GATEWAY/content/generate" \
  -H "Authorization: Bearer $TOKEN" -H 'Content-Type: application/json' \
  -d "{\"prompt\":\"Remix this reference into a travel poster\",\"style\":\"cinematic\",\"aspectRatio\":\"16:9\",\"referenceUploadIds\":[\"$REF_UPLOAD_ID\"]}")
echo "$GEN" | jq .
CONTENT_ID=$(echo "$GEN" | jq -r '.contentId')
# Then publish as in section A
```

**cURL — create with pick-from-mine only**

```bash
# REF_MINE_ID = an owned image id from GET /content/mine (draft or published)
GEN=$(curl -s -X POST "$GATEWAY/content/generate" \
  -H "Authorization: Bearer $TOKEN" -H 'Content-Type: application/json' \
  -d "{\"prompt\":\"Keep the subject, change background to desert dunes\",\"referenceContentIds\":[\"$REF_MINE_ID\"]}")
echo "$GEN" | jq .
```

**Common 400s:** ref not owned, clip instead of image, PUT not done (`no media`), over plan ref limit → `403` with `required_plan`.

---

#### A3. Image Studio — regenerate / update existing (Postman)

In-place edit of an owned **`draft`** or **`moderation_rejected`** item (same `CONTENT_ID`). Current image is always the multimodal **base**.

| Step | Postman | Expect |
|------|---------|--------|
| U0 | Have a draft `CONTENT_ID` from A1 or A2 (do **not** publish yet), or a rejected item after PATCH → `draft` | Status allows regenerate |
| U1 (optional) | Same as R1–R3 for extra refs | `REF_UPLOAD_ID` / `REF_MINE_ID` |
| U2 | `POST {{GATEWAY}}/content/{{CONTENT_ID}}/regenerate` (bodies below) | `200` — same Image Studio shape; **same** `contentId` |
| U3 | `GET {{GATEWAY}}/content/{{CONTENT_ID}}/media` | Updated image |
| U4–U6 | Publish → WS → mine detail | Same as A5–A7 |

**Postman — prompt-only regenerate**

```http
POST {{GATEWAY}}/content/{{CONTENT_ID}}/regenerate
Authorization: Bearer {{TOKEN}}
Content-Type: application/json

{
  "prompt": "Same subject, neon cyberpunk city at night"
}
```

**Postman — regenerate + extra refs**
...
{
  "prompt": "Blend lighting from the references",
  "style": "cinematic",
  "referenceContentIds": ["{{REF_MINE_ID}}"],
  "referenceUploadIds": ["{{REF_UPLOAD_ID}}"]
}
...
Omit `prompt` to reuse the stored prompt (must already be ≥ 3 chars on the content row).

**cURL — regenerate then publish**

```bash
REGEN=$(curl -s -X POST "$GATEWAY/content/$CONTENT_ID/regenerate" \
  -H "Authorization: Bearer $TOKEN" -H 'Content-Type: application/json' \
  -d '{"prompt":"Same subject, neon cyberpunk city at night"}')
echo "$REGEN" | jq .

CAPTION=$(echo "$REGEN" | jq -r '.captions[0]')
TAGS=$(echo "$REGEN" | jq -c '.hashtagSets[0]')

curl -s -X POST "$GATEWAY/content/$CONTENT_ID/publish" \
  -H "Authorization: Bearer $TOKEN" -H 'Content-Type: application/json' \
  -d "{\"title\":\"Neon Remix\",\"caption\":$(echo "$CAPTION" | jq -Rs .),\"hashtags\":$TAGS}" | jq .
```

**Do not use** `POST .../retry-generation` for this path — that is only for `generation_failed` and accepts **no** body/refs.  
**Do not** call regenerate on `published` (MVP returns `400`).
...
- [ ] Environment: `GATEWAY`, `TOKEN`, `CONTENT_ID`, `UPLOAD_URL`, `REF_UPLOAD_ID`, `REF_MINE_ID`
- [ ] Browser `/dev/coach-ws` connected before publish (and before generate if testing WS generation events)
- [ ] Happy path **A**: generate → media → publish → WS → mine/`published`
- [ ] Variant **A2**: upload-url + PUT and/or mine pick → generate with `referenceUploadIds` / `referenceContentIds` → publish
- [ ] Variant **A3**: regenerate draft (prompt-only and/or extra refs) → media → publish (not on `published`)
...
- [ ] FREE plan: generate/regenerate ≤ 10/day, upload-url ≤ 20/day, refs ≤ 2 (otherwise `403` + `required_plan`)
- [ ] On reject: `PATCH /content/{{CONTENT_ID}}` and/or `regenerate`, then publish again; watch a second `content:moderation_complete`
...
| Variation | Body highlights |
|-----------|-----------------|
| Prompt only | `{ prompt, style?, aspectRatio? }` |
| Create + pick from mine | `{ prompt, referenceContentIds: ["…"] }` |
| Create + fresh uploads | upload-url + PUT first, then `{ prompt, referenceUploadIds: ["contentId"] }` |
| Create + both | `{ prompt, referenceContentIds, referenceUploadIds }` |
...
```json
{
  ...
  "style": "cinematic",
  "aspectRatio": "16:9",
  "referenceContentIds": ["019f650e-b873-7115-8d9f-69301175e892"],
  "referenceUploadIds": ["019f6510-aaaa-7115-8d9f-69301175e892"]
}
```
...
| `referenceContentIds` | Optional UUID[]; owned image content from `GET /content/mine` |
| `referenceUploadIds` | Optional UUID[]; owned content ids from Journey B `upload-url` (same as `contentId`/`assetId`) |

Combined unique reference ids: FREE max **2**, PRO+ max **4**. Each must be owned, non-deleted, image mime, with `storageKey` present (PUT completed for uploads). Provenance stored on `content.generation_reference_ids`.
...
| HTTP | Condition |
|------|-----------|
| `400` | Validation / bad or unusable references |
| `403` | Daily generation limit (10) or reference count over plan — body includes `required_plan`, `upgrade_url` |

**Provider:** `AI_IMAGE_PROVIDER` — preferred GCP: `vertex` (Gemini Image + Vertex text captions). Alternatives: `gemini`, `pollinations`, `openai`, `mock`. Multimodal refs are supported on **vertex** / **gemini**; other providers ignore attached images.
...
**Purpose:** Update an **existing** Image Studio image **in-place** (same `contentId`, overwrite media + refresh captions/hashtags).

**Precondition:** Owner; status `draft` or `moderation_rejected`; `storageKey` present. Not for `published` (MVP — use create/fork later).

**Variations:**

| Variation | Body |
|-----------|------|
| Prompt only | `{ "prompt": "same subject, neon night" }` — current image sent as multimodal **base** |
| Prompt + mine refs | `{ "prompt": "…", "referenceContentIds": ["…"] }` |
| Prompt + fresh uploads | upload-url + PUT, then `{ "prompt": "…", "referenceUploadIds": ["…"] }` |
| Keep prompt, add refs | omit `prompt` (reuses stored prompt) + reference arrays |
| Style / aspect only | `{ "style": "meme", "aspectRatio": "1:1" }` with or without new prompt |

**Request body** (all fields optional, but effective prompt must be ≥3 chars after merge with stored prompt):

```json
{
  "prompt": "Same subject, cyberpunk city at night",
  "style": "cinematic",
  "aspectRatio": "16:9",
  "referenceContentIds": ["019f650e-b873-7115-8d9f-69301175e892"],
  "referenceUploadIds": []
...
Counts toward the daily generation limit. Extra refs follow the same FREE/PRO caps as generate. Current image is always attached as `role: base`.

**Errors:**

| HTTP | Condition |
|------|-----------|
| `400` | Wrong status, missing media, missing/short prompt, bad refs |
| `403` | Plan limit or not owner |
| `404` | Not found |

**Diff vs `retry-generation`:** retry is failure-only, no body, no new refs. Regenerate is the intentional edit API.
